"""Wipe demo seeds, re-insert 6 real matches from chat.db, push full
conversations + voice profile."""
...
# ---------- 1. Wipe existing matches + conversations for Julian ----------
print("=== 1. Wipe existing rows ===")
s, _ = call("DELETE", f"/clapcheeks_matches?user_id=eq.{JULIAN}")
print(f"  matches: HTTP {s}")
s, _ = call("DELETE", f"/clapcheeks_conversations?user_id=eq.{JULIAN}")
print(f"  conversations: HTTP {s}")
...
for phone, base in ROSTER_BASE.items():
    ...
    s, body = call("POST", "/clapcheeks_matches", payload)
