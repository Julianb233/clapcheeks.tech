...
export async function generateCoaching(supabase: SupabaseClient, userId: string) {
  ...
  for (const r of rows) {
    ...
    byPlatform[r.platform].swipes += r.swipes_right || 0
    byPlatform[r.platform].matches += r.matches || 0
  }
  ...
  const message = await anthropic.messages.create({
    ...
    max_tokens: 1024,
    system: `You are a dating performance coach for Clap Cheeks, a dating app optimization platform. Analyze these anonymized metrics and give 3 specific, actionable tips to improve results. Be direct and tactical, not generic. Reference actual numbers from the stats.
