...
export default defineSchema({
  ...
  people: defineTable({
    ...
    // Computed by recalibrateCadenceForOne from the last 30d of messages.
    // cadence_overrides.min/max_reply_gap_ms are clamped at 0.7x / 1.4x of
    // the observed median outbound→inbound reply gap. Updated weekly.
    cadence_overrides: v.optional(v.object({
      min_reply_gap_ms: v.number(),                 // floor: wait at least this long before next outbound
      max_reply_gap_ms: v.number(),                 // ceiling: if longer than this, nudge
      computed_at: v.number(),                      // unix ms — last calibration run
      sample_pairs: v.number(),                     // number of reply pairs used for median
    })),
    // Hour ints (0-23) where her inbound volume exceeds 1/3 of the peak hour.
    // Derived from the same 30d window. Used by fireOne to prefer active hours.
