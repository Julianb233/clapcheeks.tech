...
import SettingsForm, { type Persona, type UserSettings } from './settings-form'

export const metadata: Metadata = {
  title: 'Settings — Clapcheeks',
  description: 'Tune the AI voice, drip rules, calendar, weekly reports, and approval gates — all in one place.',
}
...
export default async function SettingsPage() {
  ...
  // Pull AI settings (persona / drip / dates / approval gates).
  ...
  const settings: UserSettings = {
    ...
    dateCalendarEmail: row?.date_calendar_email ?? user.email ?? '',
    dateSlots: ((row?.date_slots as string[] | null) ?? []),
    dateSlotDaysAhead: row?.date_slot_days_ahead ?? 14,
    dateSlotDurationHours: Number(row?.date_slot_duration_hours ?? 2),
    dateTimezone: row?.date_timezone ?? 'UTC',
